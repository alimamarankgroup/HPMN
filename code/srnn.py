...
class shan(srnn_dmem):
    ...
    def build_graph(self):
        ...
        with tf.name_scope('user_short_atten'):
            user_part, _, _ = self.attention(short_rep_user, short_rep_user, target_user, user_short_mask)
        ...
        with tf.name_scope('item_short_atten'):
            item_part, _, _ = self.attention(short_rep_item, short_rep_item, target_item, item_short_mask)
    ...
    def eval(self, step, dataset, prefix, epoch):
        ...
        preds = []
        losses = []
        for _, data in CroppedLoader(dataset, self.batchsize):
            labels += data[0]
            feed_dict = {
                self.user_mem: data[1],
                self.user_que: data[2],
